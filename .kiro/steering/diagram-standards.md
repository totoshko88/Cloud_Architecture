...
**Distinct exit points for a multi-edge side (thirds, not adjacent fractions).** When two or more edges leave the *same side* of one node, space their contact points by at least a **third** of the side (`0.25 / 0.5 / 0.75`, or top/upper-third/lower-third) — never `0.5` and `0.6`, which read as one doubled line at the glyph. Push the third edge onto a **different face** (e.g. the bottom) rather than crowding a third point onto the same side. Example: `app → cache` exits right-upper-third, `app → db` exits right-lower-third, `app → object-store` exits the **bottom**.

**A bottom fan-out exits DOWN first, then steps (stair from the bottom).** An edge leaving a node's **bottom** goes **straight down** into its lane first and only then turns sideways — it does not leave from a bottom corner and immediately veer. (Mirror of the side stair: side → step out then turn; bottom → drop then turn.)
