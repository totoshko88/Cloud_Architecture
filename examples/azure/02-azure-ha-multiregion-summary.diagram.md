---
id: azure-ha-multiregion-summary-v1
title: Azure HA Multi-Region — Summary
...
diagram_class: flow
detailed_view: 02-azure-ha-multiregion-landscape
tags:
  - azure
  - ha
  - multi-region
  ...
  - flow-summary
related_docs:
  - 02-azure-ha-multiregion-landscape.diagram.md
---

# Azure HA Multi-Region — Summary

## Overview

This is the `flow`-class summary of the Azure highly available, active-passive
multi-region reference workload in Subscription contoso-prod. It answers one question — how does a
...
DNS failover, which routes to the active region (eastus) and holds the passive
region (westus2) on standby behind health checks. Each region runs a load
...
denote the live primary flow. Every node uses a resolved Azure provider icon and a
...
This summary pairs with `02-azure-ha-multiregion-landscape.drawio` — the `landscape`-class as-built of the same
