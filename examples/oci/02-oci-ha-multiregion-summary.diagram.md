---
id: oci-ha-multiregion-summary-v1
title: OCI HA Multi-Region — Summary
...
diagram_class: flow
detailed_view: 02-oci-ha-multiregion-landscape
tags:
  - oci
  - ha
  - multi-region
  ...
  - flow-summary
related_docs:
  - 02-oci-ha-multiregion-landscape.diagram.md
---

# OCI HA Multi-Region — Summary

## Overview

This is the `flow`-class summary of the OCI highly available, active-passive
multi-region reference workload in Compartment acme-prod. It answers one question — how does a
...
DNS failover, which routes to the active region (us-ashburn-1) and holds the passive
region (us-phoenix-1) on standby behind health checks. Each region runs a load
...
denote the live primary flow. Every node uses a resolved OCI provider icon and a
...
This summary pairs with `02-oci-ha-multiregion-landscape.drawio` — the `landscape`-class as-built of the same
