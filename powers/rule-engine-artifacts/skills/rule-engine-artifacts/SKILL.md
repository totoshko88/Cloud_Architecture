...
its CLI is missing, then copies the rules/mappings/schema/hooks into the current
workspace:
...
rule-engine-init                    # copies steering + mappings + schema + hooks
...
package** (so it works with no repo checkout — this is the normal power/pip
case); the installed engine repo root; then the cloned power repo. Do not skip
this step and do not hand-copy a subset — the whole `mappings/` tree (including
